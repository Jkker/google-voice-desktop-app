...
const getCode = (title) => {
    const noCode = title.match(/code|码|OTP/i) === null;
    if (noCode) return;
...
//     showMainWindow();
// });
ipcMain.on("notification-created", (event, args) => {
    const code = args.body ? getCode(args.body) : undefined;
    if (code) {
        clipboard.writeText(code);
        args.title = `Copied ${code} @ ${args.title}`;
    }
        ...
        },
        {
            ...
            submenu: [
                ...
                },
                {
                    ...
                    click: () => {
                        shell.openExternal(
                            constants.URL_GITHUB_FEATURE_REQUEST
                        );
                    },
                },
                {
                ...
                { type: "separator" },
                {
                    ...
                    click: () => {
                        shell.openExternal(
                            constants.URL_GITHUB_SECURITY_POLICY
                        );
                    },
                },
                { type: "separator" },
